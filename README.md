...
- **Working time per Werktag (§3 ArbZG)**: a split dashboard tile with the
  ...
  planned side by side, plus a chart on the goal page pairing the last **and the
  next** 12, 6, 3 and 1 months against the 8 h average and the 10 h single-day
  cap. The forward half runs on the forecast and stops at the planning horizon
  (months without a plan are left out instead of diluting the average), so an
  overloaded plan shows up before the time is worked. Werktage are Monday to
  Saturday without public holidays; vacation is left out entirely so it neither
  lifts nor lowers the average
