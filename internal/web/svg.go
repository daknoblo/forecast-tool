...
// workloadColor picks the column colour by how close a month sits to the 8 h
// limit: green with room left, orange from 90 % on, red once exceeded.
func workloadColor(pct float64, over bool) string {
	switch {
	case over:
		return "#dc2626"
	case pct >= 90:
...
// workloadTimelineSVG draws the average working time per Werktag month by month
// around today: history on the left, the plan on the right, today marked in
// between. Booked months are solid, planned ones translucent and dashed - the
...
func workloadTimelineSVG(t forecast.WorkloadTimeline) template.HTML {
	const (
		...
		h        = 300.0
		padL     = 44.0
		padR     = 76.0 // room for the limit labels on the right
		padT     = 40.0 // room for the legend above the plot
		padB     = 50.0
		colLimit = "#dc2626"
		colLong  = "#94a3b8"
		colToday = "#1d4ed8"
	...
	centerX := func(i int) float64 { return padL + colW*(float64(i)+0.5) }
	barW := colW * 0.56
	baseY := padT + plotH
	...
	fmt.Fprintf(&b, `<line x1="%g" y1="%g" x2="%g" y2="%g" stroke="#94a3b8"/>`, padL, baseY, padL+plotW, baseY)
	...
	for i, m := range t.Months {
		...
		if m.HasData {
			top := y(m.PerDay)
			col := workloadColor(m.PerDay/forecast.WorkdayLimitHours*100, m.Over)
			style := ""
			if m.Ahead {
