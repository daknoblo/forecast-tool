...
const DefaultMonthPlanningPrompt = `Plane die vorhandenen Wochenstunden als realistische tägliche Buchungsblöcke im angefragten Monat. Lerne aus den tatsächlichen Tagesbuchungen der letzten zwölf Wochen: typische Blockgröße, Anzahl belegter Tage, freie Tage, wechselnde Wochentage und mehrwöchige/asynchrone Rhythmen. 8 Stunden pro Woche können beispielsweise vier Blöcke zu je 2 Stunden sein, nicht automatisch fünfmal 1,6 Stunden. Verteile nicht gleichmäßig ohne historische Evidenz. Berücksichtige die Kontinuität derselben assignmentKey über Geschäftsjahre hinweg; Projekt-IDs bleiben dennoch getrennt. Unterscheide Urlaub, Feiertage und eingeschränkte Projektfenster von echten Arbeitsmustern. Nutze bei wenig Projekthistorie vorsichtig das persönliche Muster. Erhalte die vorgegebenen Stunden je Projekt und Woche exakt, ändere keine Vergangenheit und keine Nachbarmonate. Fasse auf Deutsch ausschließlich die gewählte Tagesverteilung je Projekt zusammen. Konkrete Gründe für nicht unterbringbare Stunden gehören nur in unallocated.reason.`

const MonthPlanningExplanationFormat = `Darstellung des Feldes explanation als Planungsübersicht: Schreibe ausschließlich die getroffenen Planungsentscheidungen auf Deutsch, als Markdown-Aufzählung mit genau einem Stichpunkt je geplantem Projekt. Beginne mit dem fett hervorgehobenen Projektnamen (**Projektname**), gefolgt von den geplanten Tagen bzw. Zeiträumen und den zugehörigen Stunden oder Buchungsblöcken. Beschreibe die Verteilung in zwei bis drei kurzen, vollständigen Sätzen pro Projekt: insgesamt eingeplante Stunden im bearbeiteten Zeitraum, konkrete Tage oder zusammengefasste Zeiträume und die jeweiligen Blockgrößen. Benenne Unterschiede zwischen den Wochen oder einzelne abweichende Tage, sofern vorhanden; erfinde keine Unterschiede und wiederhole keine Zahlen ohne Mehrwert. Fasse gleiche Tagesverteilungen kompakt zusammen, ohne wichtige Details wegzulassen. Nenne nur Ergebnisse, die durch entries und unallocated belegt sind; nicht verteilte Stunden gegebenenfalls knapp mit Menge, ohne Begründung. Keine Einleitung, kein Fazit, keine allgemeinen Hinweise zu Vorschau, Speichern, Urlaub oder Feiertagen. Keine Erklärungen, historischen Muster, Herleitungen, Regeln, Unsicherheitsdiskussionen, Auslastungsstatistiken oder rechtlichen Hinweise. Konkrete Gründe für nicht verteilbare Stunden stehen ausschließlich in unallocated.reason. Diese Darstellungsvorgabe hat für explanation Vorrang vor anderen Aufforderungen, Muster, Regeln oder Abweichungen zu erläutern; die Planungs- und Validierungsregeln selbst bleiben unverändert. Kein HTML, keine Bilder und keine Links. Diese Markdown-Formatierung gilt ausschließlich INNERHALB des JSON-Strings explanation; die gesamte Antwort bleibt ein gültiges JSON-Objekt ohne umschließenden Markdown-Codeblock. Kodiere Zeilenumbrüche im JSON-String korrekt als \n.`

const MonthPlanningSystemPrompt = `Du erstellst ausschließlich eine Vorschau, niemals eine Speicherung. Antworte ausschließlich mit einem JSON-Objekt ohne umschließenden Markdown-Codeblock:
{"entries":[{"date":"YYYY-MM-DD","projectId":"ID","hours":2}],"unallocated":[{"weekStart":"YYYY-MM-DD","projectId":"ID","hours":2,"reason":"Begründung"}],"explanation":"Kurze deutsche Planungsübersicht"}
...
Die Projektliste ist KEINE Planungsliste. history enthält auch gelegentliche Ist-Buchungen von Projekten ohne Forecast. Ein Projekt darf in einer Woche nur geplant werden, wenn weeks.projects für genau diese projectId und weekStart positive editableHours enthält. Fehlt dieser Eintrag oder sind editableHours null/0, plane dafür weder entries noch unallocated. Leite aus historischen Buchungen, Projektaktivität oder assignmentKey niemals zusätzliche Wochenstunden ab. Die Historie bestimmt nur die Verteilung bereits vorhandener Forecast-Stunden, nicht deren Menge oder Projektauswahl.
