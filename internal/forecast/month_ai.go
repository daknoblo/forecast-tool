...
const MonthPlanningExplanationFormat = `Darstellung des Feldes explanation: Schreibe auf Deutsch mit kurzen Absätzen und einer Markdown-Aufzählung, genau ein eigener Stichpunkt je geplantem Projekt. Beginne jeden Projekt-Stichpunkt mit dem fett hervorgehobenen Projektnamen (**Projektname**), gefolgt von erkanntem Buchungsmuster, gewählter Tagesverteilung und gegebenenfalls Unsicherheit. Allgemeine Hinweise zu Urlaub, Feiertagen, Auslastung oder nicht verteilbaren Stunden stehen getrennt in kurzen Absätzen. Kein zusammenhängender Textblock, kein HTML, keine Bilder und keine Links. Diese Markdown-Formatierung gilt ausschließlich INNERHALB des JSON-Strings explanation; die gesamte Antwort bleibt ein gültiges JSON-Objekt ohne umschließenden Markdown-Codeblock. Kodiere Zeilenumbrüche im JSON-String korrekt als \n.`
